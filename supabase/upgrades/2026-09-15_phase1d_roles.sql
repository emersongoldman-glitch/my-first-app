-- ============================================================================
-- Campus Rooms — UPGRADE: guide-or-student confirmation at first sign-in.
--
-- For a project that has already run 2026-09-15_phase1c_rooms.sql.
-- Paste the whole file into the SQL Editor and Run. Safe to re-run.
--
-- Everyone confirms their role once. "Guide" is granted only if the typed
-- email matches the signed-in account AND is on staff_allowlist, which staff
-- manage from the Manage guides page. Existing guides/admins are backfilled
-- as confirmed and added to the list.
--
-- GENERATED from supabase/migrations/20260915001300_role_confirmation.sql
-- ============================================================================
...
-- ==== verification ================================================
-- Expect: allowlisted_guides ≥ 5 | staff_confirmed = staff_total | confirm_role_installed 1
select
  (select count(*) from staff_allowlist)                                              as allowlisted_guides,
  (select count(*) from profiles where role in ('guide','admin') and role_confirmed)  as staff_confirmed,
  (select count(*) from profiles where role in ('guide','admin'))                     as staff_total,
  (select count(*) from pg_proc p join pg_namespace n on n.oid = p.pronamespace
    where n.nspname = 'public' and p.proname = 'confirm_role')                        as confirm_role_installed;
