-- ============================================================================
-- Campus Rooms — UPGRADE: 2-hour booking horizon + app_now().
--
-- For a project that has already run 2026-09-15_phase1.sql. Paste the whole
-- file into the SQL Editor and Run. Safe to re-run.
--
-- Students may now book at most 2 hours ahead (PLAN.md D10). Staff exempt.
--
-- GENERATED from supabase/migrations/20260915001100_booking_horizon.sql
-- ============================================================================
...
-- ==== verification ================================================
-- Expect: horizon_minutes 120 | old_horizon_rows 0 | app_now_installed 1 | app_now_is_live 1
select
  (select (value #>> '{}')::int from settings where key = 'booking_horizon_minutes') as horizon_minutes,
  (select count(*) from settings where key = 'booking_horizon_days')                 as old_horizon_rows,
  (select count(*) from pg_proc p join pg_namespace n on n.oid = p.pronamespace
    where n.nspname = 'public' and p.proname = 'app_now')                            as app_now_installed,
  (select (abs(extract(epoch from (public.app_now() - now()))) < 1)::int)           as app_now_is_live;
