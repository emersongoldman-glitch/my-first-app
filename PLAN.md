...
| D17 | **Conference rooms are booked by seat.** A `shared` room allows overlapping bookings until its seats are used up; pods stay exclusive. Capacity is enforced in the database under a per-room lock, the way the exclusion constraint protects pods. Conf 1 & 2 = 4 seats, Conf 3 = 8, Conf 4 = 6 (unconfirmed). Conference zone is the 2nd floor. | 2026-09-16 | A conference room with one person in it is mostly empty. Seats let it serve several groups, and the board shows what's left. |
