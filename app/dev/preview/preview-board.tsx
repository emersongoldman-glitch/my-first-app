...
function build(now: number) {
  const zones = {
    ...
    conf:      { id: 5, name: 'Conference', floor: 2, sort: 50 },
  ...
  const mk = (id: number, name: string, z: keyof typeof zones, capacity = 1, kind: Room['kind'] = 'pod'): Room => ({
    id, slug: name.toLowerCase().replace(/\s+/g, '-'), name, capacity, kind, max_minutes: null, bookable: true, shared: kind === 'conference', sort: id, zones: zones[z],
  })
  const rooms: Room[] = [
    ...
    mk(12, 'Conference Room 1', 'conf', 4, 'conference'), mk(13, 'Conference Room 2', 'conf', 4, 'conference'),
    mk(14, 'Conference Room 3', 'conf', 8, 'conference'), mk(15, 'Conference Room 4', 'conf', 6, 'conference'),
  ...
  const bk = (id: string, room_id: number, from: number, to: number, status: Booking['status'], name: string, user_id = 'u-' + name, seats = 1): Booking => ({
    id, room_id, user_id, booked_by: user_id, seats, during: range(from, to), purpose: null, status, checked_in_at: null,
  ...
  const bookings: Booking[] = [
    ...
    bk('b3', 14, -30, 150, 'checked_in', 'Kent', 'u-Kent', 3),
    bk('b9', 14, -10, 50, 'reserved', 'Aarya', 'u-Aarya', 2),
